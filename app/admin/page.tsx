...
function SessionsTab() {
  ...
  useEffect(()=>{load()},[])
  const save=async()=>{
    const body={...form,id:editing,sort_order:Number(form.sort_order)||nextOrder()}
    ...
    if(!res.ok){const e=await res.json().catch(()=>({error:'Ошибка сохранения'}));setMsg(e.error||'Ошибка сохранения');return}
  ...
  const move=async(id:number,dir:-1|1)=>{
    ...
    setItems(sortSessions(items.map(s=>s.id===current.id?{...s,sort_order:other.sort_order}:s.id===other.id?{...s,sort_order:current.sort_order}:s)))
    const headers={'Content-Type':'application/json'}
    const a=await fetch('/api/sessions',{method:'PATCH',headers,body:JSON.stringify({id:current.id,sort_order:other.sort_order})})
    const b=await fetch('/api/sessions',{method:'PATCH',headers,body:JSON.stringify({id:other.id,sort_order:current.sort_order})})
    if(!a.ok||!b.ok)setMsg('Не удалось поменять порядок')
    await load()
  }
  return <section><Title sub="Даты, порядок, руководители RU/EN/ET, метка 'мест мало' и статус 'укомплектована'. Сортировка всегда идёт по номеру порядка.">Смены</Title><Toast msg={msg}/><div style={S.card}><div style={{display:'grid',gridTemplateColumns:'repeat(2,1fr)',gap:12}}><Field label="Порядок" v={String(form.sort_order||'')} set={v=>setForm({...form,sort_order:v})}/><Field label="Даты" v={form.dates} set={v=>setForm({...form,dates:v})}/><Field label="Руководители RU" v={form.leaders_ru} set={v=>setForm({...form,leaders_ru:v})}/><Field label="Leaders EN" v={form.leaders_en} set={v=>setForm({...form,leaders_en:v})}/><Field label="Juhendajad ET" v={form.leaders_et} set={v=>setForm({...form,leaders_et:v})}/><Field label="Название RU" v={form.type_ru} set={v=>setForm({...form,type_ru:v})}/><Field label="Название EN" v={form.type_en} set={v=>setForm({...form,type_en:v})}/><Field label="Название ET" v={form.type_et} set={v=>setForm({...form,type_et:v})}/><div><label style={S.label}>Тип</label><select style={S.input} value={form.detail} onChange={e=>setForm({...form,detail:e.target.value})}><option value="surf">Серфинг</option><option value="kino">Серфинг + Кино</option><option value="pohod">Серфинг + Поход</option></select></div><Field label="Цвет" v={form.color} set={v=>setForm({...form,color:v})}/><div style={{display:'flex',gap:18,alignItems:'center',paddingTop:24}}><label><input type="checkbox" checked={form.hot} onChange={e=>setForm({...form,hot:e.target.checked})}/> Мест мало</label><label><input type="checkbox" checked={form.sold_out} onChange={e=>setForm({...form,sold_out:e.target.checked})}/> Укомплектована</label></div></div><button style={{...S.btn('#0B3D6B'),marginTop:14}} onClick={save}>{editing?'Сохранить':'Добавить смену'}</button></div>{sortSessions(items).map((s,i,rows)=><div key={s.id} style={{...S.card,display:'flex',alignItems:'center',gap:12}}><div style={{display:'grid',gap:6}}><button style={S.btn('#e0f2fe','#0B3D6B')} disabled={i===0} onClick={()=>move(s.id,-1)}>↑</button><button style={S.btn('#e0f2fe','#0B3D6B')} disabled={i===rows.length-1} onClick={()=>move(s.id,1)}>↓</button></div><div style={{width:34,height:34,borderRadius:10,background:'#eef7fb',color:'#0B3D6B',display:'grid',placeItems:'center',fontWeight:900}}>{s.sort_order}</div><div style={{width:8,height:50,borderRadius:8,background:s.color}}/><div style={{flex:1}}><strong>{s.dates}</strong><div style={{fontSize:13,color:'#6B8AA0'}}>{s.type_ru} · {s.leaders_ru||s.leaders} {s.hot?'· мест мало':''} {s.sold_out?'· укомплектована':''}</div></div><button style={S.btn('#e0f2fe','#0B3D6B')} onClick={()=>edit(s)}>Изменить</button><button style={S.btn('#fee2e2','#dc2626')} onClick={()=>del(s.id)}>Удалить</button></div>)}</section>
}
