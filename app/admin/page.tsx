...
function SessionsTab() {
  const blank = {dates:'',type_ru:'',type_en:'',type_et:'',color:'#1A6BAA',leaders:'',hot:false,sold_out:false,detail:'surf',sort_order:0}
  ...
  useEffect(()=>{load()},[])
  const save=async()=>{const body={...form,id:editing,sort_order:Number(form.sort_order)||nextOrder()}; await fetch('/api/sessions',{method:editing?'PATCH':'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)}); setForm(blank); setEditing(null); setMsg('Смена сохранена'); load(); setTimeout(()=>setMsg(''),2500)}
  const edit=(s:Session)=>{setEditing(s.id);setForm({dates:s.dates,type_ru:s.type_ru,type_en:s.type_en,type_et:s.type_et,color:s.color,leaders:s.leaders,hot:s.hot,sold_out:!!s.sold_out,detail:s.detail,sort_order:s.sort_order})}
