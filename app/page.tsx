...
export default function Home() {
  ...
  const DATES = [
    { dates: '15.06 – 19.06.2026', type: c('СЕРФИНГ + КИНО','SURF + CINEMA','SURF + KINO'), color: '#7C3AED', leaders: 'Наташа К. + Даша', hot: true, hasDetail: true },
    { dates: '29.06 – 03.07.2026', type: c('СЕРФИНГ + КИНО','SURF + CINEMA','SURF + KINO'), color: '#7C3AED', leaders: 'Наташа К. + Даша', hot: false, hasDetail: true },
    { dates: '06.07 – 10.07.2026', type: c('СЕРФИНГ ЛАГЕРЬ','SURF CAMP','SURFI LAAGER'), color: '#1A6BAA', leaders: 'Надежда + Григорий', hot: false, hasDetail: false },
    { dates: '13.07 – 17.07.2026', type: c('СЕРФИНГ + ПОХОД','SURF + HIKE','SURF + MATK'), color: '#16A34A', leaders: 'Виталий + Григорий', hot: false, hasDetail: true },
    { dates: '20.07 – 24.07.2026', type: c('СЕРФИНГ ЛАГЕРЬ','SURF CAMP','SURFI LAAGER'), color: '#1A6BAA', leaders: 'Надежда + Ксения', hot: false, hasDetail: false },
    { dates: '27.07 – 30.07.2026', type: c('СЕРФИНГ (4 ДНЯ)','SURF (4 DAYS)','SURF (4 PÄEVA)'), color: '#1A6BAA', leaders: 'TBD', hot: false, hasDetail: false },
    { dates: '03.08 – 07.08.2026', type: c('СЕРФИНГ ЛАГЕРЬ','SURF CAMP','SURFI LAAGER'), color: '#1A6BAA', leaders: 'Даша + …', hot: false, hasDetail: false },
    { dates: '10.08 – 14.08.2026', type: c('СЕРФИНГ ЛАГЕРЬ','SURF CAMP','SURFI LAAGER'), color: '#1A6BAA', leaders: 'Надежда + …', hot: false, hasDetail: false },
    { dates: '17.08 – 21.08.2026', type: c('СЕРФИНГ + ПОХОД','SURF + HIKE','SURF + MATK'), color: '#16A34A', leaders: 'Виталий + …', hot: false, hasDetail: true },
  ]
  ...
  const CSS = `
    ...
    *{box-sizing:border-box;margin:0;padding:0}
    ...
    .rv{opacity:0;transform:translateY(18px);transition:opacity 580ms var(--ease),transform 580ms var(--ease)}
    .rv.on{opacity:1;transform:translateY(0)}
    .sg>*{opacity:0;transform:translateY(14px);transition:opacity 480ms var(--ease),transform 480ms var(--ease)}
    .sg.on>*:nth-child(1){opacity:1;transform:translateY(0);transition-delay:0ms}
    .sg.on>*:nth-child(2){opacity:1;transform:translateY(0);transition-delay:70ms}
    .sg.on>*:nth-child(3){opacity:1;transform:translateY(0);transition-delay:140ms}
    .sg.on>*:nth-child(4){opacity:1;transform:translateY(0);transition-delay:210ms}
    .sg.on>*:nth-child(5){opacity:1;transform:translateY(0);transition-delay:280ms}
    .sg.on>*:nth-child(6){opacity:1;transform:translateY(0);transition-delay:350ms}
    .sg.on>*:nth-child(7){opacity:1;transform:translateY(0);transition-delay:420ms}
    .sg.on>*:nth-child(n+8){opacity:1;transform:translateY(0);transition-delay:490ms}
    ...
    .sec-h-lt{color:white} .sec-h-lt em{color:var(--teal-lt)}
    ...
    .btn-teal{background:var(--teal);color:white;padding:14px 32px;box-shadow:0 4px 20px rgba(10,172,172,.28)}
    .btn-teal:hover{background:var(--teal-lt);transform:translateY(-1px);box-shadow:0 8px 32px rgba(10,172,172,.4)}
    .btn-sun{background:var(--sun);color:white;padding:14px 32px;box-shadow:0 4px 20px rgba(245,166,35,.25)}
    ...
    .nav.on{background:rgba(11,61,107,.97);border-bottom:1px solid rgba(255,255,255,.08);backdrop-filter:blur(24px)}
    ...
    .nav-logo{width:40px;height:40px;border-radius:50%;object-fit:cover;flex-shrink:0}
    ...
    .hero-glow{position:absolute;width:800px;height:800px;background:radial-gradient(ellipse,rgba(10,172,172,.12) 0%,transparent 65%);top:-200px;right:-150px;pointer-events:none}
    ...
    .hero-dot{width:6px;height:6px;border-radius:50%;background:var(--teal);flex-shrink:0}
    ...
    .pill{font-size:12px;font-weight:600;color:rgba(255,255,255,.75);padding:6px 14px;background:rgba(255,255,255,.1);border:1px solid rgba(255,255,255,.15);border-radius:50px}
    .hero-acts{display:flex;gap:12px;flex-wrap:wrap;margin-bottom:40px}

    /* HERO CARD */
    .hcard{background:rgba(255,255,255,.06);border:1px solid rgba(255,255,255,.12);border-radius:16px;overflow:hidden;backdrop-filter:blur(12px)}
    ...
    .hcard-row{display:flex;align-items:center;padding:10px 0;border-bottom:1px solid rgba(255,255,255,.05)}
    ...
    .hstat{padding:16px 18px;text-align:center;border-right:1px solid rgba(255,255,255,.06)}
    .hstat:last-child{border-right:none}
    .hstat-n{font-family:'Plus Jakarta Sans',sans-serif;font-size:26px;font-weight:800;color:white;line-height:1;margin-bottom:3px;letter-spacing:-.02em}
    ...
    .trust-it{padding:44px 28px;text-align:center;border-right:1px solid var(--bwarm)}
    .trust-it:last-child{border-right:none}
    .trust-n{font-family:'Plus Jakarta Sans',sans-serif;font-size:46px;font-weight:800;color:var(--ocean);line-height:1;letter-spacing:-.03em;margin-bottom:6px}
    ...
    .fcard-banner::after{content:'';position:absolute;inset:0;background:linear-gradient(to top,rgba(0,0,0,.5) 0%,transparent 60%);pointer-events:none}
    ...
    /* SAFETY */
    .safety{background:linear-gradient(135deg,#062040,#0B3D6B);padding:88px 0}
    ...
    .pc-feat{background:var(--ocean);border:2px solid var(--ocean)}
    ...
    .includes{background:white;border:2px solid var(--border);border-radius:16px;padding:28px}
    ...
    /* WHY */
    .why{background:var(--ocean);padding:96px 0}
    ...
    .why-it:hover{background:rgba(255,255,255,.04)}
    ...
    .dc:hover{transform:translateY(-2px);box-shadow:0 12px 36px rgba(11,61,107,.08);border-color:var(--teal)}
    .dc.hot{border-color:var(--sun);background:rgba(245,166,35,.03)}
    .dc-badge{font-size:9px;font-weight:700;letter-spacing:.14em;text-transform:uppercase;color:var(--sun);background:var(--sun-pale);padding:4px 10px;border-radius:20px;display:inline-block;margin-bottom:10px}
    ...
    /* SCHEDULE */
    .sched{background:var(--ocean2);padding:96px 0}
    ...
    .sched-it:hover{background:rgba(255,255,255,.04)}
    ...
    .gi{aspect-ratio:4/3;border-radius:10px;overflow:hidden;transition:transform 240ms var(--ease)}
    ...
    .rf-star{font-size:24px;transition:transform 150ms}
    ...
    .faq-it{background:var(--sand-lt);border:2px solid var(--bwarm);border-radius:12px;overflow:hidden;transition:border-color 200ms}
    ...
    .loc-card{background:white;border:2px solid var(--border);border-radius:12px;padding:14px 16px;display:flex;gap:12px;align-items:flex-start}
    ...
    /* CTA */
    .cta{background:linear-gradient(135deg,#082847,#0B3D6B,#0E5A8A);padding:110px 0;position:relative;overflow:hidden}
    ...
    <>
      ...
      <section className="trust">
        <div className="wrap">
          <div className="trust-g sg">
            {[
              {n:'7+', l:c('лет школа работает','years running','aastat töötame')},
              {n:'1000+', l:c('детей прошли лагерь','kids attended','last osalenud')},
              {n:'3', l:c('уникальных программы','unique programmes','ainulaadset programmi')},
              {n:'9', l:c('смен за лето','sessions per summer','vahetust suvel')},
            ].map(t => (
              <div key={t.l} className="trust-it">
      ...
      <section className="formats" id="formats">
        <div className="wrap">
          <div className="fhead rv">
            <div className="tag tag-teal">{c('Наши программы','Our programmes','Meie programmid')}</div>
          ...
          <div className="fg sg">
            {[
              {
                bg:'linear-gradient(145deg,#1A0A3A,#3D1B6B)',
                ...
                leaderPhoto:'/IMG_9532-150x150.jpeg',
              ...
              {
                bg:'linear-gradient(145deg,#062A14,#0E5028)',
                ...
                leaderPhoto:'/IMG_7046-150x150.jpg',
              ...
              {
                bg:'linear-gradient(145deg,#062040,#0B3D6B)',
              ...
              <div key={i} className="fcard">
                <div className="fcard-banner" style={{background:f.bg}}>
                ...
                {expandedFormat === i && (
                  <div className="fcard-exp">
                    ...
                    <div className="fcard-leader">
                      {f.leaderPhoto
                        ? <img src={f.leaderPhoto} className="fcard-lphoto" alt={f.leaderName}/>
                        : <div style={{width:44,height:44,borderRadius:'50%',background:'var(--ocean)',display:'flex',alignItems:'center',justifyContent:'center',flexShrink:0,color:'white',fontWeight:800,fontSize:18}}>{f.leaderName[0]}</div>
                      }
                      <div>
      ...
      <section className="team" id="team">
        <div className="wrap">
          ...
          <div className="team-g sg">
            {[
              {photo:'/IMG_9532-150x150.jpeg', name:'Наталья Карасёва', role:c('Ведущая — Серфинг + Кино','Cinema programme lead','Kino-programmi juht'), bio:c('Тележурналист с 20-летним стажем в двух странах, автор подкаста «Cozy with Tasha», создатель документальных проектов. Учит детей видеть историю в каждом кадре и не бояться камеры.','TV journalist with 20+ years in two countries, podcast "Cozy with Tasha". Teaches kids to see a story in every frame.','Teleajakirjanik 20+ aastat, taskuhäälingu autor, dokumentaalfilmide looja.')},
              {photo:'/IMG_7046-150x150.jpg', name:'Виталий Холстинин', role:c('Ведущий — Серфинг + Поход','Hiking programme lead','Matkaprogrammi juht'), bio:c('Предприниматель, хайкер, основатель Join The Hike. Прошёл Höga Kusten (140 км, UNESCO) и Land of Giants (120 км). Живёт тем, что преподаёт.','Entrepreneur, hiker, founder of Join The Hike. Completed Höga Kusten (140km, UNESCO) and Land of Giants (120km).','Ettevõtja, matkaja, Join The Hike asutaja.')},
              {photo:null, name:'Надежда + Григорий', role:c('Инструктора по серфингу','Surf instructors','Surfiinstruktorid'), bio:c('Сертифицированные инструктора с многолетним опытом работы с детьми на Балтийском море. Умеют мотивировать и поддерживать — от первого шага на доске до уверенного катания.','Certified surf instructors with years of experience with children on the Baltic Sea. They know how to motivate and support at every stage.','Sertifitseeritud instruktorid mitmeaastase Läänemere kogemusega.')},
            ].map((t,i) => (
              <div key={i} className="tc">
                {t.photo
                  ? <img src={t.photo} className="tc-av" alt={t.name}/>
                  : <div className="tc-av-p">{t.name[0]}</div>
                }
            ...
            {DATES.map((d,i) => (
              <div key={i} className={`dc${d.hot?' hot':''}`}>
                ...
                <div className="dc-acts">
                  ...
                  {d.hasDetail && <button className="dc-more" onClick={() => go('formats')}>{c('Подробнее','Details','Loe lähemalt')}</button>}
                </div>
              </div>
            ))}
          </div>
      ...
      <section className="sched" id="program">
        <div className="wrap">
          ...
          <div className="sched-g sg">
            {[
              {time:'09:00', name:c('Сбор','Arrival','Kogunemine'), desc:c('Встреча детей, знакомство, настрой на день','Meeting kids, introductions, setting the mood','Laste vastuvõtt, tutvumine')},
              {time:'09:30', name:c('Разминка','Warm-up','Soojendus'), desc:c('Активные тренировки и командные игры на берегу','Active training and team games on shore','Aktiivsed treeningud ja meeskonnmängud')},
              {time:'10:00', name:c('Водный блок','Water block','Vesiplokk'), desc:c('Теория безопасности, гидрокостюмы, SUP и виндсерфинг','Safety theory, wetsuits, SUP and windsurfing','Ohutusteooroia, märjaksüidid, SUP')},
              {time:'12:00', name:c('Обед','Lunch','Lõuna'), desc:c('Индивидуальные порции с учётом аллергий от Tark Catering','Individual portions, allergies noted — Tark Catering','Individuaalsed portsjonid — Tark Catering')},
              {time:'13:30', name:c('Активная программа','Active programme','Aktiivne programm'), desc:c('Виндсёрфинг, кайт, SUP, поход или пляжные игры — зависит от погоды','Windsurfing, kite, SUP, hiking or beach games — depends on weather','Purjelaud, kait, SUP, matk või rannamängud')},
              {time:'15:30', name:c('Чаепитие','Tea time','Tee aeg'), desc:c('Перерыв, отдых, свободное общение','Break, rest, free conversation','Paus, puhkus, vaba suhtlemine')},
              ...
              {time:'17:00', name:c('До завтра!','See you tomorrow!','Homseni!'), desc:c('Конец программы, ожидание родителей','Programme ends, parents pick up','Programm lõpeb, vanemad tulevad järele')},
            ].map(s => (
      ...
      <section className="gallery" id="gallery">
        <div className="wrap">
          ...
          <div className="gg sg">
            <div className="gi gi-wide"><img src="/photo-output-2-1024x1024__1_.jpeg" alt="Camp group"/></div>
            <div className="gi"><img src="/DSC02878-150x150.jpeg" alt="Windsurfing"/></div>
            <div className="gi"><img src="/IMG_7917-1024x768.jpeg" alt="Team"/></div>
            <div className="gi"><img src="/DSC02601-150x150.jpeg" alt="SUP surfing"/></div>
            <div className="gi"><img src="/DSC02899-150x150.jpeg" alt="Water"/></div>
            <div className="gi"><img src="/IMG_8779-1-768x1024.jpeg" alt="Windsurfing 2"/></div>
            <div className="gi"><img src="/IMG_6615-821x1024.jpeg" alt="Wing"/></div>
            <div className="gi"><img src="/IMG_9585-150x150.jpeg" alt="Group photo"/></div>
            <div className="gi"><img src="/IMG_9302-150x150.jpeg" alt="Team activities"/></div>
            <div className="gi"><img src="/IMG_9294-150x150.jpeg" alt="Beach"/></div>
            <div className="gi"><img src="/15-300x257.png" alt="Waves"/></div>
            <div className="gi"><img src="/IMG_0843-150x150.jpeg" alt="Base"/></div>
            <div className="gi"><img src="/IMG_0806-150x150.jpeg" alt="Surf"/></div>
            <div className="gi"><img src="/IMG_0857-150x150.jpeg" alt="Briefing"/></div>
            <div className="gi"><img src="/IMG_0850-150x150.jpeg" alt="Camp life"/></div>
            <div className="gi gi-wide"><img src="/217650841_4107074432694657_6267790752617918985_n.jpg" alt="Kite beach"/></div>
          </div>
        </div>
      </section>
      ...
      <section className="reviews" id="reviews">
        <div className="wrap">
          ...
          <div style={{textAlign:'center'}}>
            {!showReviewForm ? (
              <button className="btn btn-outline" onClick={() => setShowReviewForm(true)}>{c('Оставить отзыв','Leave a review','Jäta arvustus')}</button>
            ) : reviewSent ? (
              ...
              <div className="rev-form">
                ...
                <div style={{display:'flex',gap:10,justifyContent:'flex-end'}}>
                  <button className="btn btn-outline btn-sm" onClick={() => setShowReviewForm(false)}>{c('Отмена','Cancel','Tühista')}</button>
                  <button className="btn btn-teal btn-sm" onClick={submitReview} disabled={reviewLoading}>
                    {reviewLoading ? c('Отправка...','Sending...','Saadan...') : c('Отправить','Submit','Saada')}
                  </button>
                </div>
              </div>
            )}
